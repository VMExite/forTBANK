...
public class ScrapperGrpcClient {
    ...
    public void registerChat(long chatId) {
        chatClient.registerChat(
            ScrapperProto.RegisterChatRequest.newBuilder()
                .setId(chatId)
                .build()
        );
    }
    ...
    public void deleteChat(long chatId) {
        chatClient.deleteChat(
            ScrapperProto.DeleteChatRequest.newBuilder()
                .setId(chatId)
                .build()
        );
    }
    ...
    public ListLinkResponse getLinks(long chatId) {
        ScrapperProto.ListLinkResponse response = linkClient.getLinks(
            ScrapperProto.GetLinkRequest.newBuilder()
                .setTgChatId(chatId)
                .build()
        );
        return new ListLinkResponse(
            response.getLinksList().stream().map(this::mapLink).toList(),
            response.getSize()
        );
    }

    public LinkResponse addLink(long chatId, AddLinkRequest request) {
        ScrapperProto.LinkResponse response = linkClient.createLink(
            ScrapperProto.AddLinkRequest.newBuilder()
                .setTgChatId(chatId)
    ...
    public LinkResponse removeLink(long chatId, RemoveLinkRequest request) {
        ScrapperProto.LinkResponse response = linkClient.deleteLink(
            ScrapperProto.RemoveLinkRequest.newBuilder()
                .setTgChatId(chatId)
                .setLink(request.link())
                .build()
        );
